...
bool Lemming::apexTransition(Coord c, bool apex) {
    if (apex || m_bounceTarget <= m_bounceDistance + 1) {
        Coord next = getTargetCoord(c, getDirection());
...
void Lemming::doSomething() {
        ...
        // 1.check current square climbable
    {
        ...
        Coord above = getTargetCoord(here, up);
        bool apex = false;
        if (apexTransition(here, apex)) return;
        if (moveUp(above)) {
            m_bounceDistance++;
            return;
        }
